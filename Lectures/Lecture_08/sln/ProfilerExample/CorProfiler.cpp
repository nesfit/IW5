...
HRESULT STDMETHODCALLTYPE CorProfiler::JITCompilationStarted(FunctionID functionId, BOOL fIsSafeToBlock)
{
    ...
    IfFailRet(metadataImport->GetMethodProps(token, nullptr, nullptr, 0, &nameBufferLength, nullptr, nullptr, nullptr, nullptr, nullptr));
    // Allocate buffer for method name
    WCHAR* nameBuffer = new WCHAR[nameBufferLength];
    // Copy method name
    IfFailRet(metadataImport->GetMethodProps(token, nullptr, nameBuffer, nameBufferLength, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr));

    if (wcscmp(nameBuffer, L"Main") == 0)
    {
        CComPtr<IMetaDataEmit> metadataEmit;
        CComPtr<IMethodMalloc> methodMalloc;
        IfFailRet(metadataImport->QueryInterface(IID_IMetaDataEmit, reinterpret_cast<void**>(&metadataEmit)));
        IfFailRet(corProfilerInfo->GetILFunctionBodyAllocator(moduleId, &methodMalloc));

        ULONG size = 0;
        LPCBYTE methodHeader = nullptr;       
        IfFailRet(corProfilerInfo->GetILFunctionBody(moduleId, token, &methodHeader, &size));

        // Allocate new method
        BYTE newSize = (BYTE)(size + 1);
        LPBYTE newMethod = (LPBYTE)methodMalloc->Alloc(size + 2);
        // Copy original implementation
        std::memcpy(newMethod, methodHeader, size);
        // Rewrite last instruction RET to BR.S -12
        // Branch to first instruction of the method
        newMethod[size - 2] = 0x2B;
        newMethod[size - 1] = -12;

        IfFailRet(corProfilerInfo->SetILFunctionBody(moduleId, token, newMethod));
    } 
