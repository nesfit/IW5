...
public class Consent : PageModel
{
    ...
    public async Task<IActionResult> OnPost()
    {
        ...
        var request = await _interaction.GetLoginRequestByInternalIdAsync(Input.Id, CancellationToken.None);
        ...
        else if (Input?.Button == "yes")
        {
            ...
            if (Input.ScopesConsented != null && Input.ScopesConsented.Any())
            {
                ...
                // emit event
                await _events.RaiseAsync(new ConsentGrantedEvent(User.GetSubjectId(), request.Client.ClientId, request.ValidatedResources.RawScopeValues, result.ScopesValuesConsented, false), CancellationToken.None);
            }
            else
            {
        ...
        if (result != null)
        {
            ...
            await _interaction.CompleteLoginRequestAsync(result, CancellationToken.None);
    ...
    {
        var request = await _interaction.GetLoginRequestByInternalIdAsync(id, CancellationToken.None);
        if (request != null && request.Subject.GetSubjectId() == User.GetSubjectId())
        {
